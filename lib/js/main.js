...
class Gameboard {
	...
	playCards() {
		if(this.deckArray[0].cards.length === 0 || this.deckArray[1].cards.length === 0) {
			...
			if(!this.checkWin()){
				console.log("Player 1 has "+this.deckArray[0].cards.length+" cards in their deck and Player 2 has "+this.deckArray[1].cards.length+" cards in their deck.");
			} else {
				for (let i = 0; i < this.playerCount; i++) {
			...
			//}
			setTimeout(() => { //used to go through an entire game quickly, comment out if you want to play round by round
	...
	war() { // plays [warcount +1] cards into the inPlayArray facedown (just not console.logging / console log "3 cards played facedown") and flips one card faceup for each player which also goes into the inPlayArray. uses the comparevalue function on them and returns the result
		...
		let endWarIncre = 0;
		console.log("WAR!");
		//console.log(this.inPlayArray); //bug fixes
		for (let i = 0; i < this.playerCount; i++) {
			for (let j = 0; j < this.warCount + 1; j++) {
				if (this.deckArray[i].cards.length > 1) {
					warArray.push(this.deckArray[i].draw());
				} else if (this.deckArray[i].cards.length < 2 && endWarIncre === 0) {
					console.log("Player "+(i + 1)+" played "+(j + 1)+" cards facedown.");
					endWarIncre++;
				}
			}
			endWarIncre = 0;
		...
		for (let i = this.playerCount - 1; i >= 0; i--) {
			this.inPlayArray.unshift(this.deckArray[i].draw());
		...
		//console.log(this.inPlayArray);
		for (let i = 0; i < this.playerCount; i++) {
			if (this.deckArray[i].cards.length === 0) {
				console.log("Player "+(i + 1)+ " played their last card, the "+this.inPlayArray[i].rank+" of "+this.inPlayArray[i].suit+"!");
			} else {
		...
		this.warCount++;
		//console.log("In war before concat inPlayArray: "+this.inPlayArray.length);
		this.inPlayArray = this.inPlayArray.concat(warArray);
		//console.log("In war after concat inPlayArray: "+ this.inPlayArray.length);
		let warWinner = this.compareValue(this.inPlayArray[0], this.inPlayArray[1]);
		console.log("Player "+(warWinner + 1)+" wins the war!");
		//if (this.inPlayArray[0].value !== this.inPlayArray[1]) {
	...
	returnToDeck(winnerIndex) { //takes the winning players index in deckArray and returns the inPlayArray to the bottom of the winners deck in an arbitrary order (possibly turn the inPlayArray into a deck and use the shuffle method then just use the new decks card array to add to the players deck), and resets the inPlayArray
		...
		//console.log("inPlayArray length: "+this.inPlayArray.length);
		//console.log("deckArray "+winnerIndex+" length before: "+this.deckArray[winnerIndex].cards.length);
		...
		//console.log("deckArray "+winnerIndex+" length after: "+this.deckArray[winnerIndex].cards.length);
		this.inPlayArray = [];
		this.checkWin();
	}
...
let War = new Gameboard();
console.log("To play War, pick who will be player 1 and player 2 and then type War.playCards()")
